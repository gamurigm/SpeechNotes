...
@router.get("/")
async def list_settings(_: Annotated[object, Depends(require_auth)], category: Optional[str] = None):
...
async def list_categories(_: Annotated[object, Depends(require_auth)]):
...
@router.get("/validate")
async def validate_settings(_: Annotated[object, Depends(require_auth)]):
...
@router.put("/{key}")
async def update_setting(key: str, body: SettingUpdate, _: Annotated[object, Depends(require_auth)]):
...
@router.put("/")
async def bulk_update(body: SettingBulkUpdate, _: Annotated[object, Depends(require_auth)]):
...
@router.delete("/{key}")
async def clear_setting(key: str, _: Annotated[object, Depends(require_auth)]):
